...
- **Nightly backups are a charter requirement, not optional.** Neon provides point-in-time restore / branching; on top of that, a scheduled **`pg_dump` to off-provider object storage** (so a Neon-account-level failure isn't fatal) is the belt-and-suspenders posture for a permanent artwork. 🟢 Implemented (Phase 7): `scripts/backup.mjs` dumps the **unpooled** connection (`-Fc`) and uploads to **Cloudflare R2** (S3-compatible), scheduled 03:00 UTC by `.github/workflows/backup.yml`.
- Backups are verified by periodic test-restores — an unrestorable backup is not a backup. 🟢 `scripts/restore-verify.mjs` restores each fresh dump into a throwaway `postgres:17` container in the same nightly job and asserts the page count matches source and every committed page keeps its `content_hash`.
- **Error logging/alerting** (Phase 7): `lib/monitor.ts` emits structured JSON events and, when `MONITOR_WEBHOOK_URL` is set, pushes to a Discord/Slack webhook — `db_query_failed` (the charter-critical one), `generation_failed`, `moderation_persistent_reject`, and backup-job failures. See [Operations](./operations.md).
...
- **Per-visitor rate limit (~10 new pages/min):** 🟢 enforced at admission control ([§2](#2-the-page-resolution-lifecycle) step 4), keyed by IP (no accounts exist). Cache hits and `next`/`random` to *existing* pages don't count — only generations. Implemented as a Postgres sliding-window counter (`rate_limit_hits`) in `lib/economics.ts`; the IP is **hashed** before storage and out-of-window rows are pruned, so nothing is retained long-term ([§12](#12-security)). Chosen over edge KV because the DB is already provisioned and fine at this scale.
- **Monthly spend cap (~$10):** 🟢 a running monthly spend counter (`monthly_spend`, one row per UTC month), incremented per generation by `tokens × price` (`recordSpend`). Checked before each generation (`checkAdmission`); when exceeded, the library flips to **explore-only mode** — cache hits still work, generation returns the "still-dark" response, and crucially the address is **not** crystallized so it can generate after the monthly reset. Free (`:free`) models price at 0, so the cap is armed but inert until a paid tier is added via `MODEL_PRICES`.
...
Both controls sit *after* the store lookup and *before* the LLM call (in `resolvePage`'s `generateAndCommit`), so they only ever gate the expensive path.
